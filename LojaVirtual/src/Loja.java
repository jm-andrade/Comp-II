...
public class Loja {
    ...
    public String receberPedido(Vendavel item, int quantidade, Usuario usuario) {
        ...
        if (quantidadeEmEstoquePorItem.get(item) < quantidade) {
            throw new RuntimeException("A quantidade do estoque está insuficiente");
        }
        ...
        // abatendo o estoque
        Integer qtd = this.quantidadeEmEstoquePorItem.get(item);
        this.quantidadeEmEstoquePorItem.put(item, qtd-quantidade);
    ...
    public void incluirItem(Vendavel vendavel) {
        if (buscarItem(vendavel.getId()) != null) {
            ...
            Integer qtd= this.quantidadeEmEstoquePorItem.get(vendavel);
            this.quantidadeEmEstoquePorItem.put(vendavel, qtd+1);
            return;
        }
        // incluindo item
        catalogo.add(vendavel);
        this.quantidadeEmEstoquePorItem.put(vendavel, 1);
    }
